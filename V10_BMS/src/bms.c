...
void bms_interrupt_callback(void) {
  ...
  if (sys_stat & 0x80) {
    ...
    if ((ccVal > 0 && ccVal > 2) || (ccVal < 0 && ccVal < -2)) {
      ...
      // We thought the pack was full, but it's still charging, so we need to
      // update its' size.
      ...
      // We thought the pack was empty, but it isn't, so again, we need to
      // update our estimate of what it can hold!
...
void interrupts_init() {
  ...
  // This line is designed to be possible for either device to pull it up or
  // down to indicate a fault condition, so no pullups.
  config_extint_chan.gpio_pin_pull = EXTINT_PULL_NONE;
  config_extint_chan.detection_criteria = EXTINT_DETECT_RISING;

  extint_chan_set_config(8, &config_extint_chan);
  extint_register_callback(bms_interrupt_callback, 8,
    ...
    if (cell_voltages[i] < CELL_LOWEST_DISCHARGE_VOLTAGE) {
      ...
      for (int i = 0; i < 7; ++i) {
        sprintf(debug_msg_buffer, "Cell %d: %d mV, min %d mV\r\n", i,
                cell_voltages[i], CELL_LOWEST_DISCHARGE_VOLTAGE);
        serial_debug_send_message(debug_msg_buffer);
      }
#endif
    }
  ...
  for (int i = 0; i < 7; ++i) {
    if (cell_voltages[i] < CELL_LOWEST_CHARGE_VOLTAGE) {
      ...
      sprintf(debug_msg_buffer,
              "%s: Cell %d below min charge voltage %d, min %d\r\n",
...
void bms_handle_idle() {
  // Three potential ways out of this state - someone pulls the trigger, plugs
  // in a charger, or the IDLE_TIME is exceeded and we go to sleep.
  ...
  // Reached the end of our wait loop, with nobody pulling the trigger, or
  // plugging in charger. Transit to sleep state
  bms_state = BMS_SLEEP;
}
...
void bms_handle_discharging() {
  ...
  leds_display_battery_soc((eeprom_data.current_charge_level * 100) /
  ...
  if (bms_is_safe_to_discharge()) {
    ...
    // Brief pause to allow vac to wake up before we start sending serial data
    // at it.
  }
...
#ifdef SERIAL_DEBUG
    sprintf(debug_msg_buffer,
            "Discharging at %d mA, %d mAH, capacity %d mAH, Temp %d'C\r\n",
            currentmA * -1, eeprom_data.current_charge_level / 1000,
...
void bms_handle_fault() {
  ...
  // Show the error status and continue to show it, until trigger released and
  // charger unplugged.
  do {
    if (bms_error == BMS_ERR_PACK_DISCHARGED ||
        bms_error == BMS_ERR_UNDERVOLTAGE) {
      // If the problem is just a flat pack, blink the lowest battery segment
      // three times.
      leds_show_pack_flat();
      ...
      // Flash the red error led the number of times indicated by the fault
      // code.
  ...
  } while (port_pin_get_input_level(TRIGGER_PRESSED_PIN) ||
           port_pin_get_input_level(CHARGER_CONNECTED_PIN));
...
void bms_handle_charging() {
  ...
  while (1) {
    ...
    leds_flash_charging_segment((eeprom_data.current_charge_level * 100) /
                                eeprom_data.total_pack_capacity);

#ifdef SERIAL_DEBUG
    sprintf(debug_msg_buffer,
            "Charging at %d mA, %d mAH, capacity %d mAH, Temp %d'C\r\n",
            currentmA, eeprom_data.current_charge_level / 1000,
            eeprom_data.total_pack_capacity / 1000,
            bq7693_read_temperature() / 10);
    serial_debug_send_message(debug_msg_buffer);
#endif
...
#ifdef SERIAL_DEBUG
      sprintf(debug_msg_buffer,
              "Charging paused - cell full, attempt %d of %d\r\n",
