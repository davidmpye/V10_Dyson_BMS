...
int eeprom_init(void) {
  ...
  if (error_code == STATUS_ERR_NO_MEMORY) {
    // We are here because the fuses are set to 0x07, meaning eeprom is not
    // enabled. Show a few slow flashes to make it clear we're up to something,
    // then reprogram fuses and reset the mcu.
    for (int i = 0; i < 4; ++i) {
    ...
    // we'll fine tune this by charging and discharging.
    eeprom_data.total_pack_capacity =
        2000000; // in microAmpHours - equiv of 2000mAh.
...
int eeprom_fuses_set(void) {
  ...
  // Configure the fuse bits to enable EEPROM 1024bytes - minimal size for the
  // ASF eeprom library to use. Bits 4-6 specify eeprom size. Clear bits 4-6.
  data[0] &= ~0x00000070;
  // Eeprom to 1024 bytes / 4 rows (EEPROM bits 0x04)
  data[0] |= 0x00000040;

  // Writeback sequence from
