...
const int SCD_delay_setting[4] = {70, 100, 200, 400};

const int SCD_threshold_setting[8] = {44,  67,  89,  111,
                                      133, 155, 178, 200}; // mV

const int OCD_delay_setting[8] = {8, 20, 40, 80, 160, 320, 640, 1280}; // ms
const int OCD_threshold_setting[16] = {17, 22, 28, 33, 39, 44, 50, 56,
...
static inline void pin_set_peripheral_function(uint32_t pinmux) {
  ...
  PORT->Group[port].PMUX[((pinmux >> 16) - (port * 32)) / 2].reg &=
      ~(0xF << (4 * ((pinmux >> 16) & 0x01u)));
  PORT->Group[port].PMUX[((pinmux >> 16) - (port * 32)) / 2].reg |=
      (uint8_t)((pinmux & 0x0000FFFF) << (4 * ((pinmux >> 16) & 0x01u)));
}
...
void bq7693_init() {
  bq7693_i2c_init();
  bq7693_write_register(
      SYS_CTRL2,
      0x00); // Ensure that charge/discharge FETs are off so pack is safe.
  ...
  bq7693_adc_gain =
      365 + (((scratch1 & 0x0C) << 1) | ((scratch2 & 0xE0) >> 5)); // uV/LSB
  ...
  scratch1 = (((((long)CELL_OVERVOLTAGE_TRIP - bq7693_adc_offset) * 1000) /
               bq7693_adc_gain) >>
              4) &
             0xFF;
  bq7693_write_register(OV_TRIP, scratch1);

  scratch1 = (((((long)CELL_UNDERVOLTAGE_TRIP - bq7693_adc_offset) * 1000) /
               bq7693_adc_gain) >>
              4) &
             0xFF;
  ...
  bq7693_write_register(
      SYS_CTRL2,
      0x40); // CC_EN - enable continuous operation of coulomb counter

  bq7693_write_register(SYS_CTRL1, 0x18); // ADC_EN, TEMP_SEL

  bq7693_read_register(SYS_STAT, 1, &scratch1);
  bq7693_write_register(SYS_STAT,
                        scratch1); // Explicitly clear any set bits in the
                                   // SYS_STAT register by writing them back.
}

bool bq7693_read_register(uint8_t addr, size_t len, uint8_t *buf) {
  // Disable interrupts from the EIC - we don't want to end up trying to read
  // the charge counter half way through an existing i2c op. Re-enable at the
  // end.
  ...
  struct i2c_master_packet packet = {
      .address = BQ7693_ADDR, .data_length = 1, .data = &addr};
...
bool bq7693_write_register(uint8_t addr, uint8_t value) {
  ...
  // Initial write to set register address.
  struct i2c_master_packet packet = {
      .address = BQ7693_ADDR, .data_length = 3, .data = buf};
...
uint8_t bq7693_calc_checksum(uint8_t inCrc, uint8_t inData) {
  // CRC is calculated over the slave address (including R/W bit), register
  // address, and data.
  uint8_t i;
  uint8_t data;
  data = inCrc ^ inData;
...
void bq7693_enable_charge() {
  ...
  bq7693_write_register(SYS_STAT,
                        scratch); // Explicitly clear any set bits in the
                                  // SYS_STAT register by writing them back.
...
uint16_t *bq7693_get_cell_voltages() {
  ...
  for (int i = 0; i < 7; ++i) {
    // Because CRC is enabled, we need to read 3 bytes (VCx_HI, the CRC byte
    // (ignore), then VCx_Lo)
    bq7693_read_register((VC1_HI_BYTE + 2 * cellsToRead[i]), 3, scratch);
    tempval = ((scratch[0] & 0x3F) << 8) | scratch[2];
    bq7693_cell_voltages[i] =
        tempval * bq7693_adc_gain / 1000 + bq7693_adc_offset;
  }
...
int bq7693_get_pack_voltage() {
  ...
  int bq7693_pack_voltage =
      4 * bq7693_adc_gain * tempval / 1000 + (7 * bq7693_adc_offset);
